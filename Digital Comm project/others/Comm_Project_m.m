...
%Find closest constellation symbol, symbol-by-symbol.
for k1 = 1:length( noise)
    [nil,ind] = min(abs( noise(k1) - c));
    z(k1) = ind - 1;
end

zb=de2bi(z,'left-msb'); % converting from decimal to binary symbol
zb=reshape(zb.',numel(zb),1');  % converting symbol to vector of bits 0s and 1s
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% BER
ber=0; 
...
%now comparing the data from c's vector after rounding the input data.
zz = zeros(size(noise1)); 

%Find closest constellation symbol, symbol-by-symbol.
for k11 = 1:length(noise1)
    [nill,indd] = min(abs(noise1(k11)-cc));
    zz(k11)= indd- 1;
end

zbb=de2bi(zz,'left-msb'); % converting from decimal to binary symbol
zbb=reshape(zbb.',numel(zbb),1');  % converting symbol to vector of bits 0s and 1s
ber1=0; 
for i=1:length(stream)
    if stream(i)~=zbb(i)
        ber1=ber1+1;
...
q=sqrt((3/2)*EbNoratio.*(9/(M-1)));
Pe=(2/9)*(1-1/sqrt(M)).*erfc(q);
figure
